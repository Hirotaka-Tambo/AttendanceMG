...
<body>
   <div class= "container">
      ...
      <form action="users" method="post" class="user-form">
         ...
         <c:if test="${userToEdit.username}">
         </c:if>
         
         <label for="username">ユーザーID:</label>
         <input type="text" id="username" name ="username" value="<c:out value="${user.ToEdit.username}"/>"<c:if test="${userToEdit != null}">readonly</c:if> required>
         ...
         <select od="role" name="role" required>
            <option value="employee" <c:if test="${userToEdit.role == 'employee'}">selected</c:if>>従業員</option>
            <option value="admin" <c:if test="${userToEdit.role == 'admin'}">selectetd</c:if>>管理者</option>
         </select>
         
         <p>
            <label for ="enabled">アカウント有効:</label>
            <input type="checkbox" id="enabled" value="true"<c:if test="${userToEdit == null || userToEdit.enabled}">checked</c:if>>
         </p>
